...
try:
    from .modelagem import prever_pontuacao, preparar_features_historicas
except ImportError:
    # Fallback para execução direta/debug
    import sys
...
def simular_desempenho_recente(window=3, orcamento=140, formacao="4-3-3", risco=0.0, modelo_tipo="IA", alpha=0.2):
    ...
    # Precisa enriquecer os dados com features históricas (Mando, Adversário, Médias Avançadas)
    # Isso é pesado, mas necessário para precisão.
    if modelo_tipo == "IA Avançada (XGBoost)":
        # Usa a função de modelagem que já faz todo o trabalho pesado (Merges, GroupBys)
        # Precisamos passar o DF completo para calcular médias históricas corretamente
        try:
            ...
            # O XGBoost exige que posicao_id seja numérico, mas o CSV pode ter strings ('ata', 'gol').
            ...
            # Re-aplica mapeamento de clubes para exibição se necessário (preparar_features cria clube_id_num)
            # Mas o otimizador usa 'clube' string para travas (se houver).
            if 'clube' not in df_ano.columns:
                # Reaplica mapeamento simples de nomes
                 with open(CLUBS_DATA_PATH, 'r', encoding='utf8') as f:
                    clubes_map = json.load(f)
                    id_to_name = {int(k): v['nome_fantasia'] for k, v in clubes_map.items()}
            ...
            # Exibe erro na UI para debug
            try:
                st.error(f"⚠️ Erro ao ativar IA na simulação: {e}. Usando média simples.")
            except:
                pass # Pode falhar se não estiver no contexto do Streamlit
            # Fallback: continua com dados simples
            pass
    ...
    for rodada in rodadas_teste:
        ...
        if modelo_tipo == "IA Avançada (XGBoost)":
            try:
                ...
                # DEBUG VISUAL: Mostra se houve mudança
                # delta = df_r['pontuacao_prevista'].mean() - df_r['media_num'].mean()
                # print(f"Rodada {rodada}: Delta IA vs Média = {delta:.2f}")
        ...
        # Se Modelo Clássico, tenta aplicar Odds
        elif alpha > 0:
            odds_encontradas = False
            if df_odds_hist is not None:
                # Filtra odds daquela rodada/ano
                df_odds_r = df_odds_hist[(df_odds_hist['rodada_id'] == rodada) & (df_odds_hist['ano'] == ano_max)]
                if not df_odds_r.empty:
                    # Merge simples (assumindo nomes batem)
                    # Precisaríamos de lógica de confronto aqui, mas simplificando:
                    # Se não tivermos o confronto no df_hist, é difícil.
                    # O df_hist não tem 'adversario'. 
                    # Para MVP, vamos assumir que sem 'adversario' no historico, não aplicamos odds.
                    # Melhoria futura: salvar confrontos no historico.
                    pass 
            
            # Como historico atual não tem adversário salvo, ignoramos odds no backtest por enquanto
            # para evitar erros. Assume-se neutro.
            pass
...
def gerar_comparativo_historico(orcamento=140, formacao="4-3-3", risco=0.0):
    ...
    for _, row in df_vini.iterrows():
        ...
        pontos_max = 0
        try:
            # Maximiza a pontuação REAL (mas respeita o orçamento para ser justo)
            # Se quiser o absoluto, aumente o orçamento aqui
            time_max = otimizar_escalacao(
                df_r, 
                coluna_pontos='pontuacao', # O PULO DO GATO: Maximiza o que realmente aconteceu
                coluna_preco='preco_num',
                orcamento_total=orcamento + 20, # Dá uma folga pro time perfeito
                formacao_t_str=formacao,
                fator_risco=0.0 # Risco não importa aqui
            )
            # Adiciona capitão (dobra o maior pontuador real)
            if not time_max.empty:
                # Remove técnico para capitão
                ...
                if not jogadores_linha.empty:
                    ...
                    pontos_max = time_max['pontuacao'].sum() + max_pts_jogador # Dobra o capitão
                else:
